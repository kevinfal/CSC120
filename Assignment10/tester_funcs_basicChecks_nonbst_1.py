...
# we use the same values and tree as the search_1 testcase.
from tester_TreeNode_search_1 import get_tree
...
nodes, and tests count() and is_bst() at every node."""
    ...
    while next < len(nodes):
       ...
       print("    count(): {}".format(count(this_node)))
       print("    is_bst(): {}".format(is_bst(this_node)))
